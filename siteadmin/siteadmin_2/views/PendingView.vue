...
<script>
import pendingList from '../src/components/pendingList'
...
export default {
  ...
  methods:{
    ...
    searchPending(){
      if(this.searchInput !== "")
      {
        axios.get("http://localhost:7777/searchforpending/"+this.searchInput)
        .then(res => this.users = res.data)
        .catch(err => console.log(err));
      }else{
        axios.get("http://localhost:7777/showpendingusers/")
        .then(res => this.users = res.data)
        .catch(err => console.log(err))
      }
    },
    approveUser(id){
      axios.get("http://localhost:7777/approvepending/"+id)
      .then()
      .catch(err => console.log(err))
      this.users = this.users.filter(user => user[0] !== id);
    }
